...
"""
OpenSwitch Test for RBAC create user with ops_admin role.
"""
...
def test_chap01_basic(topology, step):
    ...
    configuration = ('show running-configuration').split("\n")
    for line in configuration:
        if "aaa authentication login radius radius-auth pap" not in line:
            rc = True
    assert rc, "Failed - Verify PAP configuration"
    ...
    configuration = ('show running-configuration').split("\n")
    for line in configuration:
        if "aaa authentication login radius radius-auth chap" not in line:
            rc = True
    assert rc, "Failed - Verify chap configuration"
