...
def setupradiusclient(step):
    ...
    assert (
        "Unknown command" not in out, "Failed to enter configuration terminal"
    )
...
def localauthenable(step):
    ...
    assert (
        "Unknown command" not in out, "Failed to enable local authentication"
    )
    out += s1("echo ", shell="bash")
    s1("exit")
    return True
...
def radiusauthenable(step, chap=False):
    ...
    if chap:
        ...
        assert (
            "Unknown command" not in out, "Failed to set chap for radius"
        )
    else:
        out += s1("echo ", shell="bash")
        out = s1("aaa authentication login radius")
        assert (
            "Unknown command" not in out, "Failed to enable radius "
            "authentication"
        )
...
def nofallbackenable(step):
    ...
    assert (
        "Unknown command" not in out, "Failed to disable fallback to local"
        "authentication"
    )
...
def fallbackenable(step):
    ...
    assert (
        "Unknown command" not in out, "Failed to enable fallback to"
...
def loginsshlocal(step):
    ...
    sshkeygen = pexpect.spawn(sshkey)  # noqa
