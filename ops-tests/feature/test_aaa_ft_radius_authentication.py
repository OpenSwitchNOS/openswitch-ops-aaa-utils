...
def stop_radius_service(step, host_idx):
    ...
    assert ("Stopping RADIUS server") in out, "Failed to stop freeradius on host " + host_ip
...
def start_radius_service(step, host_idx):
    ...
    assert ("Starting RADIUS server") in out, "Failed to start freeradius on host " + host_ip
...
def create_new_user(step, new_user, new_pass, auth_type, priv_lvl, host_idx):
    ...
    assert ("No such file or directory") not in out, \
            "Failed to add new user " + new_user + " to freeradius.conf"

def print_freeradius_conf(step, host_idx):
    step("#### print clients.conf content ####")
    h = hosts[host_idx]
    out = h("cat /etc/freeradius/clients.conf")
    print(out)
    step("#### print users content ####")
    out = h("cat /etc/freeradius/users")
    print(out)
...
def init_radius_server(step):
    ...
    create_new_user(step, USER_1, USER_1_PASSWD, 'pap', 'Nas-Prompt-User', 0)
    create_new_user(step, USER_NEW, USER_NEW_PASSWD, 'chap', 'Administrative-User', 1)
...
def setup_radius_client(step):
    ...
    s1("radius-server host " + host_2_ip_address + " key testing123 auth-type chap")
