...
The default AAA authentication server group priority information (local 0) present in `show aaa authentication` command output(table).
...
The AAA authentication local information (local 1) present in `show aaa authentication` command output(table).
#### Test Fail Criteria
