...
def add_default_row():
    ...
    setattr(ovs_rec, OPEN_VSWITCH_AUTO_PROVISIONING_STATUS_COLUMN,
            auto_provisioning_data)
...
def modify_common_auth_session_file(fallback_value, radius_value):
    ...
    for count in range(0, 2):
        ...
        if radius_value == HALON_TRUE and fallback_value == HALON_FALSE  \
           and count == 0:
            contents.insert(index, radius_auth[count] + radius_retries + "\n")

        if radius_value == HALON_TRUE and fallback_value == HALON_FALSE and  \
           count == 1:
            contents.insert(index, radius_auth[count])

        if radius_value == HALON_TRUE and fallback_value == HALON_TRUE and \
           count == 0:
            contents.insert(index, fallback_local_auth[count])
            contents.insert(index, fallback_and_radius_auth[count] +
                            radius_retries + "\n")

        if radius_value == HALON_TRUE and fallback_value == HALON_TRUE \
           and count == 1:
            contents.insert(index, fallback_local_auth[count])
            contents.insert(index, fallback_and_radius_auth[count])
