...
* Test pass criteria
1. The user “root” and the password is created on the system.
2. The “root” user starts in the bash shell.
3. The user has all permissions(SYS_MGMT, READ_SWITCH_CONFIG, WRITE_SWITCH_CONFIG).

* Test fail criteria
1. The user “root” and the password is not created on the system.
2. The “root” user does not start in the bash shell.
3. The user does not have all permissions(SYS_MGMT, READ_SWITCH_CONFIG, WRITE_SWITCH_CONFIG)
...
*Test result criteria*
* Test pass criteria
1. The user “admin” and the password is created on the system (DUT01)
...
* Test fail criteria
1. The user “admin” and the password is not created on the system (DUT01)
...
4. The user does not have network operator permissions.(READ_SWITCH_CONFIG, WRITE_SWITCH_CONFIG)
...
5. The admin user does not have read and write permissions(READ_SWITCH_CONFIG, WRITE_SWITCH_CONFIG)
...
5. The admin user has read and write permissions(READ_SWITCH_CONFIG, WRITE_SWITCH_CONFIG)
...
2. The user does not have permission(SYS_MGMT, READ_SWITCH_CONFIG, WRITE_SWITCH_CONFIG).

* Test fail criteria
1. The user belongs to admin or netop role.
2. The user has permission(SYS_MGMT, READ_SWITCH_CONFIG, WRITE_SWITCH_CONFIG).
...
*Description:*
Validate the a non-existent user does not have permissions
...
*Description:*
Validate a new user can not be assigned to multiple roles.
...
*Test result criteria*
* Test pass criteria
1. The rbac.py is installed in the appropriate location (/usr/include).
2. The librbac.so and rbac.h is installed in the appropriate locations(/usr/lib).

* Test fail criteria
1. The rbac.py is not installed in the appropriate location (/usr/include).
2. The librbac.so and rbac.h is not installed in the appropriate locations(/usr/lib).
