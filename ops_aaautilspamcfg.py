...
def  get_source_interface(protocol):
    ...
    if protocol == AAA_RADIUS:
        radius_source_interface = vrf_utils.get_radius_source_interface(idl)
        vlog.info("radius_source_interface = %s\n" % (radius_source_interface))

    if protocol == AAA_TACACS:
        tacacs_source_interface = vrf_utils.get_tacacs_source_interface(idl)
...
def get_src_ip_dstn_ns(source_interface):
    ...
    if is_v4 == True:
        ...
        src_ip = source_interface
        vlog.info("address configured: tacacs_src_ip = %s\n" % (src_ip))
    else:
        # hostname is configured
        src_ip = vrf_utils.get_ip_from_interface_name(idl, source_interface)
        vlog.info("interface configured: tacacs_src_ip = %s\n" % (src_ip))

    if src_ip is None:
        return src_ip, dstn_ns

    if src_ip != mgmt_ip:
        vrf_name = vrf_utils.get_vrf_name_from_ip(idl, src_ip)
...
def modify_common_auth_access_file(server_list):
    ...
    vlog.info("##### tacacs_src_interface = %s, radius_src_interface = %s, src_ip = %s, dstn_ns = %s ########" \
        % (str(tacacs_source_interface), str(radius_source_interface), str(src_ip), str(dstn_ns)))
            ...
            if server_type == AAA_LOCAL:
                ...
                if dstn_ns is not None:
                    auth_line = "auth\t" + PAM_CONTROL_VALUE + "\t" + PAM_TACACS_MODULE + "\tdebug server=" + ip_address + \
                        ...
                        " src_namespace=" + " dstn_namespace=" + dstn_ns +  " source_ip=" + str(src_ip) + " \n"
                else:
                    auth_line = "auth\t" + PAM_CONTROL_VALUE + "\t" + PAM_TACACS_MODULE + "\tdebug server=" + ip_address + \
                        ":" + str(tcp_port) + " secret=" + str(passkey) + " login=" + auth_type + " timeout=" + str(timeout) + "\n"
