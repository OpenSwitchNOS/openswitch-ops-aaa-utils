...
def is_ipv4(ip):
    '''
    check if the address is a ip address
    '''
    pieces = ip.split('.')
...
def extract_ip(ip):
    ...
    vlog.info("extract_ip returning ip %s " % (ip))
    return ip
...
def get_ip_from_interface_name(idl, source_interface):
    ...
    vlog.info("get_ip_from_interface_name returning ip %s " % (ip))
    return ip
...
def get_source_interface(idl):
    ...
    '''
    vlog.info("get_source_interface")
...
def get_namespace_from_vrf(idl, vrf_name):
    ...
    # is this the corrrect logic ?
    for ovs_rec in idl.tables[VRF_TABLE].rows.itervalues():
         if ovs_rec.name == vrf_name:
                namespace = ovs_rec.uuid
...
def  update_tacacs_source_interface():
    ...
    if is_v4 == True:
        ...
        vlog.info("address configured: global_source_ip_address = %s\n" % (global_source_ip_address))
    else:
        # hostname is configured
        global_source_ip_address = get_ip_from_interface_name(idl, source_interface)
        vlog.info("interface configured: global_source_ip_address = %s\n" % (global_source_ip_address))

    if global_source_ip_address is None:
        global_source_ip_address = ''
        return
            ...
            if server_type == AAA_LOCAL:
                ...
                if global_dstn_ns:
                    ...
                    auth_line = "auth\tsufficient\t" + PAM_TACACS_MODULE + "\tdebug server=" + ip_address + \
                        ...
                        " src_namespace=" + NO_NET + " dstn_namespace=" + global_dstn_ns +  " source_ip=" + str(global_source_ip_address) + " \n"
                else:
                    vlog.info("here2")
                    auth_line = "auth\tsufficient\t" + PAM_TACACS_MODULE + "\tdebug server=" + ip_address + \
                        " secret=" + str(passkey) + " login=" + auth_type + " timeout=" + str(timeout) + "\n"
