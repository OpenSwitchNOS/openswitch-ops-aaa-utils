...
def  update_tacacs_source_interface():
    ...
    vlog.info("eeentered update_tacacs_source_interface")

    mgmt_ip = vrf_utils.get_mgmt_ip(idl)
    vlog.info("mgmt_ip = %s\n" % (mgmt_ip))

    global_source_ip_address = vrf_utils.get_ip_from_interface_name(idl, "1")
    ...
    if is_v4 == True:
        ...
        if source_interface == mgmt_ip:
             global_dstn_ns = DEFAULT_NAMESPACE
        else:
            global_dstn_ns = SWITCH_NAMESPACE
    else:
