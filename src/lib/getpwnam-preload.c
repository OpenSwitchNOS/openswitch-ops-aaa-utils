...
struct passwd *getpwnam(const char *name) {
    ...
    if (pw == NULL) {
        ...
        if (pw != NULL) {
            if (strlen(pw->pw_name) < strlen(name)) {
                syslog(LOG_ERR, "Tacacs_Dev: (%s) tacacs user name "
                    "length longer than local user",
                    __FUNCTION__);
            }

            /* TODO: handle the case with longer tacacs user name */
