 ...
 * server
 */
static int initialize(radius_conf_t *conf, int accounting_val, const char *source_ip)
{
	...
	if (source_ip != NULL) {
		s_in->sin_addr.s_addr = inet_addr(source_ip);
	} else {
	...
	if (bind(conf->sockfd, &salocal, sizeof (struct sockaddr_in)) < 0) {
		...
		_pam_log(LOG_ERR, "bind: address got set successfully");
	}
...
nl_setns_with_name(config.dstn_namespace);
	...
	if (strlen(config.source_ip) > 1) {
		retval = initialize(&config, FALSE, config.source_ip);
	} else {
		retval = initialize(&config, FALSE, NULL);
	}
	PAM_FAIL_CHECK;
...
#define PAM_FAIL_CHECK if (retval != PAM_SUCCESS) {goto error; }
	...
	DPRINT(LOG_DEBUG, "Sending RADIUS request code %d", request->code);
        ...
        _pam_log(LOG_DEBUG, "radius_dstn_namespace = %s, radius source_ip = %s, dst ns len = %d, source ip len = %d",
...
static int pam_private_session(pam_handle_t *pamh, int flags, int argc, CONST char **argv, int status)
{
	 ...
	 * Then, open a socket, and bind it to a port
	 */
	retval = initialize(&config, TRUE, NULL);
	PAM_FAIL_CHECK;
