...
#define MAX_USER_ENV_LEN 273
  ...
  char remote_usr_env[MAX_USER_ENV_LEN];
	struct addrinfo *source_address = NULL;

	user = pass = tty = r_addr = NULL;
        memset(env_var_str,0,MAX_ENV_VAR_LEN);
        memset(remote_usr_env, 0, MAX_USER_ENV_LEN);
	ctrl = _pam_parse(argc, argv);
		...
		close(tac_fd);
                ...
                if (status == PAM_SUCCESS)
                {
                    ...
                    if ( priv_lvl_status == EXIT_OK ) {
                        ...
                        snprintf(remote_usr_env, MAX_USER_ENV_LEN, "%s=%s", REMOTE_USR_ENV, user);
                    } else {
