...
def handle_user_login(request):
    '''
    The request argument is an instance of class tornado.web.RequestHandler.
    This function authenticates the user & password contained in the request.
    ...
    auth = PAM.pam()
    auth.start(service)
    user = request.get_argument("name")
    ...
    try:
        ...
        request.set_secure_cookie("user", request.get_argument("name"))
        return True
